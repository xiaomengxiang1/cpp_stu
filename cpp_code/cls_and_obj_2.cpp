...
class Data {
    public:
        ...
        //const修饰成员函数
        // void Print() const -> void Print(const Data* this)
        void Print() const {
            cout << _year << "-" << _month << "-" << _day << endl;
        }
...
void f(const Data& d) {
    //d.Print(); ->  d.Print(&d)  // 编译器报错
    // void Print(Data* this) print函数中是没有const的，直接传d会放大权限，所以不能直接传,要处理print函数
    d.Print();
}
